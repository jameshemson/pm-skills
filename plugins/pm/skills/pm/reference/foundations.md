...
2. **False confidence slop**: Stating assumptions as facts, invented statistics presented as data, certainty where uncertainty exists. Its quieter form is the weasel attribution: "industry reports show", "experts agree", "research suggests", "users consistently tell us", with no source, no count, and no date. Two customers on one call is not "customers"; name the two and the call.
...
5. **Hedge slop**: "It depends on various factors..." - if you can't name the factors and their weights, you haven't done the thinking. Its newer form is the availability pivot: admit the number is missing, then reason from it anyway. "Exact churn figures are unavailable, but industry benchmarks suggest 8-10%." "Usage data is limited; adoption is likely strong among power users." The disclaimer buys licence for a guess. Fix: state what is known, name what would settle it, and stop.
...
10. **Template camouflage**: every standard section heading is present and every section is generic. The signature of AI-PRD tool output: structure used as evidence of thinking. The test is regeneration, not length - cover the heading and read the section; if the text could sit under the same heading in any product's document, or could be regenerated from the heading alone, the section is camouflage. A two-line section naming this product's specific exclusions passes; ten generic lines fail. Run the test once per document, section by section. Each camouflaged section fails the slop-test check it fakes (a hollow edge-case section fails "edge cases covered" - once, not twice). In a review, lead the slop verdict with the doc-level count ("template camouflage: N of M sections regenerable from their headings"), quote the two worst sections, and show for one of them what a specific version would say.

**Voice slop** (it sounds like AI wrote it):

11. **AI vocabulary slop**: Words like "delve", "tapestry", "landscape", "realm", "beacon", "nuanced", "multifaceted", "robust", "meticulous", "pivotal", "underscore", "intricate", "cacophony", "intricacies", "interplay", "garner", "foster", "bolster", "harness", "illuminate", "facilitate", "streamline", "showcase", "vibrant", "enduring". If the word sounds like a thesaurus suggestion, replace it with plain English. Also cut the overused qualifiers: "arguably", "it's worth noting", "importantly", "notably", "furthermore", "consequently", "ultimately".
12. **Sentence pattern slop**: "It's not X, it's Y" constructions, verb inflation ("serves as", "stands as", "represents", "marks" instead of "is"), opening by restating the question, closing with an upbeat summary paragraph. Read aloud - if it sounds like a press release, rewrite it.
13. **Formatting slop**: Excessive colon-into-list patterns, bold key phrases mid-sentence, headers every 2-3 paragraphs when unnecessary. Not everything needs a subheading or a bullet list. Never use em dashes.

14. **Assistant-artifact slop**: leftover chat scaffolding the generator forgot to strip. Four shapes. Trailing offers ("Would you like me to draft the next section?", "Let me know if you'd like me to expand this", "Is there anything else you'd like me to cover?"). Form-letter sign-offs ("I hope this helps", "I hope this finds you well"). Model self-reference ("as a large language model", "as an AI", "based on my knowledge cutoff", "I don't have personal opinions"). Unfilled placeholders ("[N]", "[TBD]", "[insert metric]", "XX%", "[Your Name]"), the generator's fill-in-the-blank template left standing where a number or a name should be. Unlike the diction tells, this is absolute: one instance anywhere fails the draft, the same standard as the em dash. A spec that ends by asking the reader whether they want a revision was pasted out of a chat window, and any reader sees it. Fix: delete every trace of the assistant turn. A document ends on its last real point. It does not offer to continue the conversation.

15. **Uniform-rhythm slop** (structural, not lexical): sentences of similar length and shape, evenly paced, paragraph after paragraph at the same size. No keyword catches this and no vocabulary swap fixes it, which is why it survives a scrub that removes every flagged word. It is the tell a reader's ear catches first, before any single word registers. The check is mechanical: read the draft aloud, or count. If three sentences in a row land within a few words of each other, or every paragraph runs three to four sentences, the meter is machine-even. Fix: vary length on purpose. Let one sentence run long and the next be three words. Break any paragraph that matches its neighbours in size.

16. **The trailing participle**: a sentence that finishes with an "-ing" phrase doing the analysis the sentence did not do. "We will ship search in Q3, enabling teams to find records faster." "The metric is read from dispatch codes, ensuring consistency with existing reporting." "Support tickets fell 12%, highlighting the need for better onboarding." The main clause states a fact; the participle bolts on a consequence, a purpose, or a significance claim with no evidence and no actor. Test: cut everything after the comma. If the sentence loses nothing you could check, the phrase was synthesis. Fix: give the claim its own sentence with a subject and a basis, or delete it. "Support tickets fell 12%. Nine of the eleven that remain are first-week setup questions, so onboarding is the next target."
...
One tell is not a verdict. Three or more from different families below means the draft reads as AI-generated and needs a voice pass before it reaches its audience. The exceptions are the mechanical tells, which need no judgement to spot and so block SHIP on one instance: the em dash, assistant-artifact slop, and the bold-label bullet list. The gates in knowledge-craft-score.md name them.
...
- Tells: CAPS mid-sentence for emphasis ("the emptiness IS the content"), bolding a key phrase mid-sentence, the compulsive rule of three (every list and sentence arriving in triads), uniform paragraph length (the standalone structural rhythm tell is Voice slop 15 above), the colon-into-list reflex, the clipped parallel negation ("Search and filters ship now. Saved views do not."), where a short second sentence mirrors and negates the first for cadence.
...
**15. The over-corrected anti-AI register.** The mirror of every tell above, and the one the skill is most at risk of producing while it removes the others. Once the smooth defaults (the em dash, "delve", "not just X, it's Y") are widely known, a second default appears: prose visibly straining not to read as AI. It is its own tell and a reader clocks it just as fast.
...
- Labelled-line tells: a label and a colon where a sentence should be. "Pilot: 20 companies with assigned CSMs, 8 weeks." "Mitigation: add a secondary provider." "Decision needed: extend or hold." And its list form, where every bullet opens with a bold label and a colon ("**Cost:** about $190 a month"). The label carries the noun and the colon replaces the verb. Wikipedia's AI-cleanup editors rank the bulleted form among the strongest formatting signs of machine text. A labelled line is fine in a table or a reference list, and an inline header with a full stop ("Cost.") is the same skeleton. In prose the tell is structural, like uniform rhythm, so count it once per document, not per line, and only when more than half the paragraphs open with a label. Two labelled openers in a one-page message are ordinary emphasis and count as nothing. The list form (three or more consecutive bullets each opening with a bold label and a colon) is mechanical and blocks SHIP on one instance. Distinct from family 13, the suspense colon before a punchline, and from family 8's colon before a list.
