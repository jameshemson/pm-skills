...
Opinionated, framework-backed product management as a set of modes. {{ASSISTANT_NAME}} generates, pm-skills critiques: every mode forces real thinking and rejects the polished, generic output that passes for PM work.
...
1. **Loaded instructions / {{INSTRUCTIONS_FILE}}**: if a **Product Context** section is already in your loaded instructions or in {{INSTRUCTIONS_FILE}}, proceed.
2. **`.pmcontext.md`**: if not, read `.pmcontext.md` from the project root. If it exists with product, users, business model, team, and technical constraints, proceed. If its `last_updated` is more than six months old, say once: "Context was captured <date> - still accurate? (`{{PM_INVOCATION}} teach` updates it; continuing as-is.)" Then proceed. No `last_updated` line means no nudge.
...
If a **Ways of Working** section exists in `.pmcontext.md` or {{INSTRUCTIONS_FILE}}, prefer the user's frameworks, checklists, and doc formats over a mode's defaults. The user's way of working takes precedence.
...
| `setup` | Generate a tailored team {{INSTRUCTIONS_FILE}} from team norms and domain | [reference/mode-setup.md](reference/mode-setup.md) |
