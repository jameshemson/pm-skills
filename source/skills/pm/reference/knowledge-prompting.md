...
The full taxonomy with detection rules and fixes is in foundations.md. The PM-specific patterns follow.
...
**Pattern**: Words that are rare in human writing but vastly overrepresented in AI output.
...
- Avoid colon-then-bullets for every explanation when prose would be clearer
- Avoid semicolons to sound sophisticated when a period works fine
