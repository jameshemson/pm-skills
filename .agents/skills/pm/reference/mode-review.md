...
The flagship mode. The user brings a document they or the assistant already wrote; `review` makes it sharper. Critique is the most common PM need, so unmatched input defaults here.
...
| OKRs, quarterly goals | [knowledge-metrics.md](knowledge-metrics.md) |
...
4. **Strategic-alignment check** (the absorbed `audit` function). Using product context, ask: does this connect to a stated outcome, or is it drift? Is the evidence strong, medium, weak, or none? For a defect fix, a reproduction by anyone, the author included, is the evidence: do not ask for customer reports, and never recommend deferring a reproduced defect until customers report it. If the document does not say how a defect was confirmed, ask it to (P2). Is this the highest-value use of the team's time? Name one hard question the team is avoiding.
...
Rank every substance finding by the severity anchors in [knowledge-craft-score.md](knowledge-craft-score.md): P0 (the document's primary action is blocked), P1 (the document must change before the audience can commit), P2 (quality erosion, and every question the audience can simply ask). Voice findings - Claudism tells and voice slop - carry no severity; they reach the verdict only through the SLOP and SHIP gates. Multi-persona findings rank above single-persona ones at the same severity.
